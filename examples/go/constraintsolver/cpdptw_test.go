...
func printSolutionCPDPTW(t *testing.T, data DataModelCPDPTW, manager RoutingIndexManager,
	...
	for n := range routing.Size() {
		...
		if solution.Value(routing.NextVar(n)) == n {
	...
	for vehicleId := 0; vehicleId < len(data.vehicleCapacities); vehicleId++ {
		...
		totalLoad += solution.Value(capacityVar)
	}
